...
```withDockerContainer()``` could be use as an alternative to ```docker.image().inside()```docker.image().inside(). See below for the synthax.
...
#### Build and run directly from a Dockerfile

For this part, I won't give you the full code of the pipeline, but if you followed the guide correctly you should now how to do the next modification for your testing. In case, to help you, I will display an example of files structure you need for this pipeline.

```sh
jenkins-agent/workspace/workspace/dockerfile-pipeline/ansible/ansible_volumes/host_interaction/
├── ansible.cfg
├── host-int-playbook.yml
└── inventory
    ├── 00_inventory.yml
...
Here, the stage you need to insert in your pipeline :

```sh
// Start the ansible playbook in the Ansible container
stage ('Build and Deployment') {
    // Assign a variable to the image build
    def ANSIBLE_IMG = docker.build (
        "test-ansible",
        "./ansible/ansible_controller"
        )
    // Following commands will be executed inside the container
    // Container generated by the image we just built
    // Go inside as root to avoid permission errors
